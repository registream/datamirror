* =============================================================================
* Test: Probit Basic - Layer 4 checkpoint constraints for probit models
* Purpose: Test probit coefficient matching with synthetic data
...
di as text "Test 1: Simple binary probit with continuous predictors"
di as text "----------------------------------------------------------"
...
* Generate binary outcome from probit model
* True coefficients: intercept = -0.3, x1 = 0.5, x2 = -0.4
gen xb = -0.3 + 0.5*x1 - 0.4*x2
gen p = normal(xb)
gen y = rbinomial(1, p)
...
local outdir "stata/tests/output/test_probit_basic/test1"
cap mkdir "stata/tests/output"
cap mkdir "stata/tests/output/test_probit_basic"
cap mkdir "`outdir'"
...
probit y x1 x2
datamirror checkpoint, tag("probit_simple")
...
di as text ""
di as text ""
di as text "Test 2: Probit with factor variables"
di as text "----------------------------------------------------------"
...
* True: base=1, level2=+0.25, level3=+0.5, level4=+0.75
gen xb = -0.6 + 0.3*x1
replace xb = xb + 0.25 if education == 2
replace xb = xb + 0.5 if education == 3
replace xb = xb + 0.75 if education == 4
gen p = normal(xb)
gen y = rbinomial(1, p)
...
* Initialize DataMirror
local outdir "stata/tests/output/test_probit_basic/test2"
cap mkdir "`outdir'"
...
* Checkpoint
probit y x1 i.education
datamirror checkpoint, tag("probit_factor")
...
* Probit uses iterative paired-swap adjustment (not direct DGP sampling).
...
if `max_dse_cont' < 3.0 {
    ...
    di as text "  Note: Factor Δβ/SE = " %6.3f `max_dse_factor' " (expected, factors not adjustable in probit engine)"
    local test2_pass = 1
}
else {
...
di as text ""
di as text ""
di as text "Test 3: Probit with multiple continuous predictors"
di as text "----------------------------------------------------------"
...
* Generate outcome
gen xb = -0.2 + 0.4*x1 - 0.3*x2 + 0.2*x3 - 0.35*x4 + 0.45*x5
gen p = normal(xb)
gen y = rbinomial(1, p)
...
* DataMirror workflow
local outdir "stata/tests/output/test_probit_basic/test3"
cap mkdir "`outdir'"

datamirror init, checkpoint_dir("`outdir'") replace
probit y x1 x2 x3 x4 x5
datamirror checkpoint, tag("probit_multi")
...
di as text ""
di as text "=========================================="
di as text "Test Summary: Probit Basic"
di as text "=========================================="
di as text ""
